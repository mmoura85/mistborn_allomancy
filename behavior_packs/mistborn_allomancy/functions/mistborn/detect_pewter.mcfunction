...
execute as @a[tag=duralumin_potion_lock,hasitem={item=mistborn:pewter_draught,location=slot.weapon.mainhand}] run title @s actionbar §cCannot drink potions while flaring Duralumin!
execute as @a[tag=duralumin_potion_lock,hasitem={item=mistborn:pewter_draught,location=slot.weapon.mainhand}] run playsound note.bass @s

# Track pewter draught in hand and detect actual drinking
execute as @a[tag=!duralumin_potion_lock,hasitem={item=mistborn:pewter_draught,location=slot.weapon.mainhand}] run scoreboard players add @s pewter_use_check 1
